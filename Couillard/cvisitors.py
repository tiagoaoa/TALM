...
class ASTPrinterVisitor(Visitor):
    ...
    def vStorageClass(self, node):
        self.pNodeInfo(node)

    def vArrayType(self, node):
        self.pNodeInfo(node)
        self.pSubnodeInfo(node.child, "Child:")
    ...
    def vSwitchStatement(self, node):
        self.pNodeInfo(node)
        self.pSubnodeInfo(node.expr, "Expression")
        self.pSubnodeInfo(node.stmt, "Statement")

    def vCaseStatement(self, node):
        self.pNodeInfo(node)
        self.pSubnodeInfo(node.const, "Constant")
        self.pSubnodeInfo(node.stmt, "Statement")
    ...
    def vSuperInstruction(self, node):
        self.pNodeInfo(node)
        self.pSubnodeInfo(node.inp, "Inputs")
...
class TypeCheckVisitor(Visitor):
    ...
    def _process_conditional(self, expr):
        ...
        if expr.type.get_outer_string() not in ['int', 'char', 'float']:
