...
reserved_words = {
    ...
    'volatile' : 'VOLATILE',
    'while' : 'WHILE',
    'treb_super': 'TREB_SUPER',
    'input': 'INPUT',
    'output': 'OUTPUT',
    'parallel': 'PARALLEL',
    'single': 'SINGLE',
    'reduce': 'REDUCE',
    'treb_parout': 'TREB_PAROUT',
    'mytid': 'MYTID',
    'local': 'LOCAL',
    'starter': 'STARTER',
    'lasttid': 'LASTTID'
...
def t_SUPERBODY(t):
    r'\#BEGINSUPER[\w\W]*?\#ENDSUPER'
    t.value = t.value.replace('#BEGINSUPER','')
    t.value = t.value.replace('#ENDSUPER','')
    t.lineno += t.value.count('\n')
    return t

def t_BLOCK(t):
    r'\#BEGINBLOCK[\w\W]*?\#ENDBLOCK'
    t.value = t.value.replace('#BEGINBLOCK','')
    t.value = t.value.replace('#ENDBLOCK','')
    t.lineno += t.value.count('\n')
    return t
