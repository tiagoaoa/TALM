...
class PreProc:
	def __init__(self):
		...
		self.replaces += [("\.g", ".0")] #go-ahead token is the output #0
		self.replaces += [("\.w", ".1")] #wait token is the output #1
		#self.replaces +=[("(\w+)\${(\w+)\.\.(\w+)}(\.\w+)*", self.taskrange)]
		self.replaces += [("([^\s,]+)\${([^}]+)\.\.([^}]+)}(\..)?", self.taskrange)] # macro for declaring a range of source operands
		#taskrage must come BEFORE taskmacro
		self.replaces += [("({(\w)+=(.*?)\.\.(.*?)}).*", self.taskmacro)] #macro for declaring multiple instructions for multiple tasks
	...
	def callgroup(self, groupname, funcname):
		...
		if not self.functioncalls.has_key(funcname): 
	...
	def evalexp(self, match):
		...
		"""
			...
			output += re.sub("(\W)%s(\W)" %var, "\\1 %d \\2" %i, line) + "\n"
			#substitute occurances of the variable by the value (0, 1, 2, 3..)

		output = re.sub("\${(.*?)}", self.evalexp, output) #evaluate the expressions using the index var.
        	return output
	...
	def varprob(self, var, prob):
		if  0 < float(prob) < 1:
			...
			print "Error: Probability for %s == 1 is not in the interval ]0;1["
	def avgtime(self, name, time):
		blocknumer = None
		if self.superinsts.has_key(name):
		...
		if 0 < int(time):
			...
			print "Error: Average time for %s is not an integer bigger than 0"
